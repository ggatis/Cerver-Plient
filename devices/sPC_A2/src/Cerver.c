...
//2nd kind of messages. msg_crc is included! what if the size is not exactly the size to fit
msg_data_t*     msg_data;
//1st kind of messages. msg_crc is hanging!
msg_header_t    rl_header;
//2nd kind of messages. msg_crc is included! what if the size is not exactly the size to fit
msg_data_t      rl_data;
...
int validate_command( char* buffer, size_t size ) {
    ...
    case TOPIC_INIT:
        ...
        transaction = rCRC;
        msg_header = (msg_header_t*)buffer;

        rl_header.msg_topic          = msg_header->msg_topic;
        rl_header.msg_kind           = msg_header->msg_kind;
        rl_header.repeats            = msg_header->repeats;
        rl_header.payload_parm_cnt   = msg_header->payload_parm_cnt;
        rl_header.total_payload_size = msg_header->total_payload_size;
    ...
    case TOPIC_DATA:
        ...
        msg_data = (msg_data_t*)buffer;
        rl_data.msg_topic = msg_data->msg_topic;
        rl_data.response  = msg_data->response;
        rl_data.current_payload_size = msg_data->current_payload_size;
        rl_data.origin_crc = msg_data->origin_crc;
        rl_data.arg_index = msg_data->arg_index;
        rl_data.chunk_index = msg_data->chunk_index;
        //apspeeleet!!!
        inc_data = (uint8_t*)( buffer + sizeof( msg_data_t ) );
...
void make_data_request( void ) {
    msg_data = (msg_data_t*)buffer;
    msg_data->msg_topic             = TOPIC_DATA;
    msg_data->response              = 0;
    msg_data->current_payload_size  = sizeof( msg_data_t ) + sizeof( msg_crc );
    msg_data->origin_crc            = transaction;
    msg_data->arg_index             = main_arg_index;
    msg_data->chunk_index           = main_chunk_index;
    msg_crc                         = crc32( 0L, (const Bytef*)buffer, (uInt)sizeof( msg_data_t ) );
    //mainaas jeega
    bytesReceived                   = msg_data->current_payload_size;
    //iemest vietaa
    memcpy( buffer + sizeof( msg_data_t ), &msg_crc, sizeof( msg_crc ) );
    printf("TX make_data_request\r\n");
    printf("msg_topic:              %d\r\n",   msg_data->msg_topic );
    printf("msg_response:           %d\r\n",   msg_data->response );
    printf("current_payload_size:   %d\r\n",   msg_data->current_payload_size );
    printf("origin_crc:             %08X\r\n", msg_data->origin_crc );
    printf("arg_index:              %d\r\n",   msg_data->arg_index );
    printf("chunk_index:            %d\r\n",   msg_data->chunk_index );
    printf("msg_crc:                %08X\r\n", msg_crc );
}
    ...
    while ( 1 ) {
        ...
        case TOPIC_DATA:
            ...
            if ( ( main_arg_index == rl_data.arg_index ) || ( main_chunk_index == rl_data.chunk_index ) ) {
                ...
                for ( uint16_t i = 0;
                    i < ( rl_data.current_payload_size - sizeof( msg_data_t ) ); )
